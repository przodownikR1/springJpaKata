...
public class JpaBulkTest {
    ...
    public void shouldBulkUpdate() {
        Query query = em.createQuery("update Address a set a.disable = true where a.id > :id").setParameter("id", 0l);
    ...
    public void shouldInsertDataInBulkWay() {
        int deleted = em.createQuery("delete Address a where a.id > 0").executeUpdate();
